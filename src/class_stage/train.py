...
def main():
    ...
    model = create_custom_model(ClassifierModel, 
    ...
    def evaluation_class(model, dataloader, eval_iters=-1):
        model.eval()
        ...
        for _step, _batch in enumerate(dataloader):
            ...
            scores = _outputs["logodds"]
            probs = sigmoid(scores)
            correct_predictions += ((probs > 0.5).float() == labels).sum()
            total_predictions += scores.shape[0]
            mean_score += scores.sum().float()
            
            if (_step + 1) == eval_iters:
                break
        
        _acc = correct_predictions / total_predictions
        mean_score = mean_score / total_predictions
    
        if args.local_rank != -1:
            _acc = get_all_reduce_mean(_acc).item()
            mean_score = get_all_reduce_mean(mean_score).item()
        
        return mean_score, _acc
    ...
    mean_score, acc = evaluation_class(
